...
llm_global_limiter = RateLimiter(max_calls=LLM_GLOBAL_LIMIT, period=LLM_GLOBAL_PERIOD)
...
SPECIAL_RESPONSES = {
    "__botinfo__": (_cached_botinfo, BOTINFO_PATH, "⚠️ Инфа потерялась, проверь путь к моему описанию"),
    "__changelog__": (_cached_changelog, CHANGELOG_PATH, "⚠️ Инфа потерялась, проверь путь к моим обновам"),
    "__commandlist__": (_cached_commandlist, COMMANDLIST_PATH, "⚠️ Инфа потерялась, проверь путь к списку команд"),
}
...
async def _handle_phrase(update: Update, context: CallbackContext, lower_text: str) -> bool:
    ...
    if response in SPECIAL_RESPONSES:
        cached, path, fallback = SPECIAL_RESPONSES[response]
        response = cached or load_text_file(path) or fallback
